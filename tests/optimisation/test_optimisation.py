...
def test_run_optimisation_no_batching(
    opt: BFGS, batch: ConformerBatch, device: str
) -> ConformerBatch:
    """Test running optimisation without batching."""
    out = run_optimisation(
        conformers=batch, optimiser=opt, batch_size=1, num_workers=0, device=device
    )
...
def test_run_optimisation_batching(opt: BFGS, batch: ConformerBatch, device: str) -> ConformerBatch:
    """Test running optimisation with batching."""
    out = run_optimisation(
        conformers=batch, optimiser=opt, batch_size=3, num_workers=0, device=device
    )
