...
def compute_strain(df: pd.DataFrame, cfg: DictConfig) -> pd.DataFrame:
    ...
    # Load data
    docked = to_mols_dict(df2, cfg.io.input.mol_col_name, cfg.io.input.id_col_name)
    ...
    md = save_parquet(df2, docked, local_minima, global_minimia, cfg.threshold, **cfg.io.output)
